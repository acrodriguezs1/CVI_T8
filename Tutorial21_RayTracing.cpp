...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    {  
    {float3{ 0, 2.45, 0}, 0.00f},   // azul
    {float3{ 0, 100, 0}, 0.53f},  // 1 
    {float3{0, 100, 0}, 1.27f},   // 2 (ahora y = 0)
    {float3{0, 100, 0}, 4.16f}    // 3 (ahora y = 0)
    };
    // clang-format on
    ...
    Instances[5].CustomId     = 1; // texture index
    Instances[5].pBLAS        = m_pProceduralBLAS;
    Instances[5].Mask         = OPAQUE_GEOM_MASK;
    Instances[5].Transform.SetTranslation(-3000.0f, -3000.0f, -5000.f);
    ...
    Instances[6].Transform.SetTranslation(3000.0f, -4000000.0f, -500000000000.0f);
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Setup camera.
    m_Camera.SetPos(float3(0, -1.8, -14.5f));
